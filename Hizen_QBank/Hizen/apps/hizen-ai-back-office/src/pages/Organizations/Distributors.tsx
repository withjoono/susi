...
const SAMPLE_DISTRIBUTORS: DistributorDto[] = [
  {
    id: '1',
    name: '에듀에이전시',
    email: 'contact@eduagency.com',
    company: '에듀에이전시 주식회사',
    address: '서울특별시 강남구 테헤란로 123',
    phone: '02-1234-5678',
    createdAt: Date.now() - 30 * 24 * 60 * 60 * 1000, // 30일 전
    approvedAt: Date.now() - 20 * 24 * 60 * 60 * 1000, // 20일 전
  ...
  },
  {
    ...
    approvedAt: undefined, // 승인 대기
    approvedBy: undefined,
  },
  {
    id: '3',
    name: '학습미래',
    email: 'hello@studyfuture.co.kr',
    company: '학습미래 유한회사',
    address: '대구광역시 중구 동성로 789',
    phone: '053-1111-2222',
    createdAt: Date.now() - 7 * 24 * 60 * 60 * 1000, // 7일 전
    approvedAt: Date.now() - 2 * 24 * 60 * 60 * 1000, // 2일 전
    approvedBy: 'admin@example.com',
  },
];
...
const Distributors = () => {
  ...
  // Fetch distributors data with sample data
  const fetchDistributors = useCallback(
    async (
      ...
      try {
        ...
        // Simple pagination with sample data
        const startIndex = (pageNumber - 1) * count;
        const endIndex = startIndex + count;
        const paginatedDistributors = SAMPLE_DISTRIBUTORS.slice(
          startIndex,
          endIndex,
        );

        setDistributors(paginatedDistributors);
        setCurrentPageToken(
          endIndex >= SAMPLE_DISTRIBUTORS.length ? null : 'next',
        );
        setIsLastPage(endIndex >= SAMPLE_DISTRIBUTORS.length);
  ...
  const handleItemsPerPageChange = (value: string | undefined) => {
    if (value && !isLoading) {
      ...
      if (newCount !== itemsPerPage) {
        ...
        fetchDistributors(1, newCount, null, null);
      }
    }
  };
  ...
  const goToNextPage = () => {
    if (isLastPage || isLoading || distributors.length === 0) return;

    const lastDistributorId = distributors[distributors.length - 1]?.id;
    if (lastDistributorId) {
      fetchDistributors(
        currentPageNumber + 1,
        itemsPerPage,
        lastDistributorId,
  ...
  const goToPreviousPage = () => {
    if (currentPageNumber <= 1 || isLoading || distributors.length === 0)
      return;

    const firstDistributorId = distributors[0]?.id;
  ...
  // --- Calculated Values ---
  const totalCount = distributors.length; // This would ideally come from the API response
  const firstItemNumber =
    totalCount === 0 ? 0 : (currentPageNumber - 1) * itemsPerPage + 1;
  const lastItemNumber = firstItemNumber + distributors.length - 1;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('영업 지사가 성공적으로 등록되었습니다.');
      setOpenDialog(false);
      setFormData({
        name: '',
        email: '',
      ...
      toast.error(`영업 지사 등록 실패: ${error.message || 'Unknown error'}`);
    } finally {
  ...
  useEffect(() => {
    fetchDistributors(1, itemsPerPage, null, null);
  }, [fetchDistributors, itemsPerPage]);

  // 승인 처리 함수
  const handleApprove = (id: string) => {
    // API implementation will be needed for approval
    toast.error('승인 API가 아직 구현되지 않았습니다.');

    // Client-side implementation for now
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    if (!window.confirm(`"${name}" 영업 지사를 삭제하시겠습니까?`)) {
      return;
    }
    ...
    try {
      ...
      toast.success('영업 지사가 성공적으로 삭제되었습니다.');
    } catch (error: any) {
      console.error('영업 지사 삭제 중 오류 발생:', error);
  ...
  return (
    <>
      ...
      <Card className="w-full">
          ...
          <div className="mb-4 flex flex-col justify-between gap-8 md:flex-row md:items-center">
            ...
            <div>
              ...
              >
                ...
                <Typography color="black">새 영업 지사 추가</Typography>
              </Button>
            </div>
          </div>
                    ...
                    <td className="py-3 px-4 border-b border-blue-gray-50">
                      <div className="w-max">
                        <Chip
                          ...
                          variant="ghost"
                          value={
                            distributor.approvedAt ? '승인됨' : '승인 대기중'
                          }
                          color={distributor.approvedAt ? 'green' : 'amber'}
                        />
                      </div>
                    </td>
                        ...
                        {!distributor.approvedAt && (
                          <Tooltip content="승인하기">
                            <IconButton
                              ...
                              size="sm"
                              onClick={() =>
                                distributor.id && handleApprove(distributor.id)
                              }
                            >
        ...
        <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
          <Typography variant="small" color="blue-gray" className="font-normal">
            ...
            <span className="font-semibold">{totalCount}</span> Distributors
          </Typography>
      ...
      <Dialog open={openDialog} handler={() => setOpenDialog(!openDialog)}>
        <DialogHeader>새 영업 지사 추가</DialogHeader>
        <DialogBody divider>
