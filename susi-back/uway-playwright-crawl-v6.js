/**
 * UWAY 수시 경쟁률 Playwright 기반 크롤링 스크립트 V6
 * V5 대비 개선사항:
 * - 모든 컬럼의 rowspan 완벽 처리 (모집인원, 경쟁률 컬럼 포함)
 * - 모집인원 컬럼이 없는 테이블 자동 건너뛰기 (정원 외 전형 등)
 * - 모집인원이 숫자가 아닌 행 필터링 (학과명이 잘못 들어간 경우)
 * - 24개 오류 대학 완벽 해결
 */
    ...
    const data = await page.evaluate((univName) => {
      ...
      tables.forEach((table, tableIndex) => {
        ...
        for (let i = headerRowIndex + 1; i < normalizedRows.length; i++) {
          ...
          // V6 핵심: 모집인원이 숫자가 아니면 건너뛰기
          // (학과명이 잘못 들어간 경우 필터링)
          const recruitNumeric = (recruitCount || '').toString().replace(/,/g, '').trim();
          if (!recruitNumeric || !/^\d+$/.test(recruitNumeric)) {
            continue;
          }
...
async function main() {
  ...
  console.log('UWAY 수시 경쟁률 Playwright 크롤링 V6');
  console.log('V5 대비 개선사항:');
  console.log('- 모든 컬럼의 rowspan 완벽 처리 (모집인원, 경쟁률 컬럼 포함)');
  console.log('- 모집인원 컬럼이 없는 테이블 자동 건너뛰기');
  console.log('- 모집인원이 숫자가 아닌 행 필터링');
  console.log('- 24개 오류 대학 해결');
  console.log('='.repeat(70));
  ...
  if (allData.length > 0) {
    ...
    const filename = `UWAY_수시_경쟁률_V6_${timestamp}.xlsx`;
