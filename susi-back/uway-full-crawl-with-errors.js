/**
 * UWAY 수시 경쟁률 Playwright 기반 크롤링 스크립트 V7
 * V6 대비 개선사항:
 * - 정원외 전형 데이터 수집 지원 (모집인원 0 허용)
 * - 모집인원이 "-", "별도", "0" 등인 경우도 수집
 * - 경쟁률 계산 개선 (모집인원 0일 때 "-" 표시)
 * - 정원외 전형 자동 감지 및 처리
 */
...
/**
 * V6 개선된 데이터 추출 함수
 * - 모든 컬럼의 rowspan 완벽 처리
 * - 모집인원 컬럼이 없는 테이블 건너뛰기
 * - 모집인원이 숫자가 아닌 행 필터링
 */
async function extractRatioDataV6(page, universityName) {
  try {
    ...
    const data = await page.evaluate((univName) => {
      ...
      /**
       * V6 핵심 개선: 모든 컬럼의 rowspan/colspan 완벽 정규화
       * 테이블의 모든 셀을 정규 2D 배열로 변환
       */
      function normalizeTableV6(table) {
        const rows = table.querySelectorAll('tr');
        const normalizedRows = [];

        // 각 열별 rowspan 추적 (모든 컬럼에 대해)
        ...
        for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
          ...
          // 이 행의 최대 컬럼 수 계산 (이전 행 기준)
          const maxCols = normalizedRows.length > 0 ?
            Math.max(...normalizedRows.map(r => r.length), 20) : 20;

          while (colIdx < maxCols) {
            // 이 컬럼에 이전 행에서 상속된 rowspan 셀이 있는지 확인
            ...
            // colspan 처리: 같은 값을 colspan 수만큼 복사
            for (let c = 0; c < colspan; c++) {
              normalizedRow.push(text);

              // rowspan 처리: 다음 행들에 이 값을 상속
              if (rowspan > 1) {
      ...
      function detectColumnMapping(headerRow) {
        ...
        headerTexts.forEach((text, idx) => {
          ...
          // 모집인원 - 정확한 매칭
          if (text === '모집인원' || text === '총모집인원' ||
              (text.includes('모집') && text.includes('인원') && !text.includes('지원'))) {
            if (mapping.recruitCount === undefined) mapping.recruitCount = idx;
          }
          // 지원인원
          if (text === '지원인원' || text === '지원자수' ||
              (text.includes('지원') && (text.includes('인원') || text.includes('자수')))) {
            if (mapping.applyCount === undefined) mapping.applyCount = idx;
          }
          // 경쟁률
          if (text.includes('경쟁률') || text === '경쟁률') {
            if (mapping.ratio === undefined) mapping.ratio = idx;
          }
        });
      ...
      function isRatioValue(text) {
        ...
        return /\d+\.?\d*\s*:\s*1/.test(trimmed) || /^\d+\.?\d*$/.test(trimmed);
      }

      /**
       * 숫자 여부 확인 (모집인원/지원인원 검증용)
       */
      ...
      tables.forEach((table, tableIndex) => {
        ...
        // V6: 완벽한 테이블 정규화
        const normalizedRows = normalizeTableV6(table);
        ...
        // V6 핵심: 모집인원 컬럼이 없으면 이 테이블 건너뛰기
        // (정원 외 특수교육대상자 등의 테이블)
        if (headerRowIndex === -1 || columnMap.recruitCount === undefined) {
          return;
        }
        ...
        for (let i = headerRowIndex + 1; i < normalizedRows.length; i++) {
          ...
          // V7 개선: 모집인원 검증 (숫자, 0, -, 별도 허용)
          const recruitText = (recruitCount || '').toString().replace(/,/g, '').trim();
          let recruitNumeric = '0';

          // 모집인원이 숫자인 경우
          if (/^\d+$/.test(recruitText)) {
            recruitNumeric = recruitText;
          }
          // "-", "별도", "정원외" 등의 경우 0으로 처리
          else if (recruitText === '-' || recruitText === '별도' || recruitText.includes('정원외')) {
            recruitNumeric = '0';
          }
          // 완전히 비어있거나 명백히 잘못된 데이터 (학과명 등)
          else if (!recruitText || recruitText.length > 10) {
            continue;
          }
...
async function crawlUniversity(browser, university, index, total) {
  ...
  try {
    ...
    const data = await extractRatioDataV6(targetPage, university.name);
...
async function main() {
  ...
  console.log('UWAY 수시 경쟁률 Playwright 크롤링 V7');
  console.log('V6 대비 개선사항:');
  console.log('- 정원외 전형 데이터 수집 지원 (모집인원 0 허용)');
  console.log('- 모집인원이 "-", "별도", "0" 등인 경우도 수집');
  console.log('- 경쟁률 계산 개선 (모집인원 0일 때 "-" 표시)');
  console.log('- 정원외 전형 자동 감지 및 처리');
  console.log('='.repeat(70));
  ...
  if (allData.length > 0) {
    ...
    const filename = `UWAY_수시_경쟁률_V7_${timestamp}.xlsx`;
