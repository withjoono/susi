...
/**
 * 종합전형 일반/특별전형 구분 엔티티
 * 테이블: susi_jonghap_special
 * 일반전형/특별전형 구분, 특별전형 유형, 지원자격
 */
@Entity('susi_jonghap_special')
@Index('idx_susi_jonghap_special_ida_id', ['idaId'], { unique: true })
@Index('idx_susi_jonghap_special_category', ['admissionCategory'])
export class SusiJonghapSpecialEntity {
  @PrimaryGeneratedColumn()
  id: number;
