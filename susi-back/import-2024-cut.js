...
async function run() {
  ...
  // 1. 엑셀 로딩
  console.log('=== 2024 실제컷 엑셀 로딩 ===');
  const wb = XLSX.readFile('uploads/2024정시-실제컷-정리.xlsx');
  ...
  // 기존 2024년 데이터 삭제
  await client.query('DELETE FROM ts_regular_admission_previous_results WHERE year = 2024');
  console.log('기존 2024년 데이터 삭제 완료');
  ...
  const result = await client.query(`
    SELECT COUNT(*) FROM ts_regular_admission_previous_results WHERE year = 2024
  `);
  console.log('2024년 레코드 수:', result.rows[0].count);
  ...
  sample.rows.forEach((r, i) => {
    ...
    console.log(`   환산50%:${r.min_cut} 환산70%:${r.max_cut} 총점:${r.converted_score_total} 백분위70%:${r.percentile_70}`);
  });

  // 매칭 안된 항목
  if (unmatched.length > 0) {
    const fs = require('fs');
    fs.writeFileSync('uploads/2024-unmatched.json', JSON.stringify(unmatched, null, 2), 'utf8');
    console.log('\n매칭 안된 항목 저장: uploads/2024-unmatched.json');
